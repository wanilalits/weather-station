    ...
    try {
      ...
      if (data[0].deviceId && !data[0].deviceId.startsWith('frontend') && !ws.deviceId.startsWith('frontend')) {
        ...
        wss.clients.forEach((client: any) => {
          if (client.deviceId && client.deviceId.startsWith('frontend') && client.readyState === WebSocket.OPEN) {
...
const clientSizeBroadcast = () => {
 wss.clients.forEach((client: any) => {
          if (client.deviceId && client.deviceId.startsWith('frontend') && client.readyState === WebSocket.OPEN) {
          client.send(JSON.stringify([{connectedClients : wss.clients.size}]));
          }
        });
};
