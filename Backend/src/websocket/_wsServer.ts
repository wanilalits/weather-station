...
// 🔥 Broadcaston frontEnd
const broadcastToFrontend = (payload: any) => {

  wss.clients.forEach((client: any, ws:any) => {
    // 1. Check if the connection is open
    // 2. Check if the userId exists and starts with 'frontend'
       console.log('📢',  client.readyState, client.userId  );
    if (client.readyState === WebSocket.OPEN && ws.userId?.startsWith('frontend')) {
